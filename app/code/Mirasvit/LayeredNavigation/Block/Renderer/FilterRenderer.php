...
class FilterRenderer extends GenericFilterRenderer
{
	...
	if ($this->filter->getRequestVar() === 'cat' || $this->filter->getRequestVar() === 'category_ids') {
            $rendererBlock = $this->_layout->createBlock(CategoryRenderer::class);
        }

        if ($this->filter->getRequestVar() === ExtraFilterConfigProvider::SEARCH_FILTER_FRONT_PARAM) {
            $rendererBlock = $this->_layout->createBlock(SearchRenderer::class);
        }
