...
class Collection extends GenericCollection
{
    ...
    public function getExtendedFacetedData(string $field, bool $exclude = false, int $allowedValue = null): array
    {
        ...
        /** @var AttributeConfigRepository $attrConfigRepository */
        $attrConfigRepository = ObjectManager::getInstance()->get(AttributeConfigRepository::class);

        $attrConfig = $attrConfigRepository->getByAttributeCode($field);

        if ($attrConfig && $attrConfig->getMultiselectLogic() === AttributeConfigInterface::MULTISELECT_LOGIC_AND) {
            try {
                return parent::getFacetedData($field);
            } catch (\Exception$e) {
                return [];
    ...
    private function createCriteriaBuilder(string $field, int $allowedValue = null): SearchCriteriaBuilder
    {
        ...
        $filterBuilder = ObjectManager::getInstance()->create(\Magento\Framework\Api\FilterBuilder::class);
        $scopeConfig   = ObjectManager::getInstance()->create(\Magento\Framework\App\Config\ScopeConfigInterface::class);
        ...
        if ($priceRangeCalculation) {
            ...
            $newCriteriaBuilder->addFilter($filterBuilder->create());
        }
