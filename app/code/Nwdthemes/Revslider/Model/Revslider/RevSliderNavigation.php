...
class RevSliderNavigation extends RevSliderFunctions {
		...
		if($raw == false){
			foreach($navigations as $key => $nav){
				...
				if(isset($navigations[$key]['settings'])){
					$navigations[$key]['settings'] = RevSliderFunctions::stripslashes_deep(FA::json_decode($navigations[$key]['settings'], true));
					if(!is_array($navigations[$key]['settings'])){
						$navigations[$key]['settings'] = FA::json_decode($navigations[$key]['settings'], true);
					}
				}
			}
		}
				...
				if($raw == false){
					foreach($def as $key => $nav){
						...
						if(isset($def[$key]['settings'])){
							$def[$key]['settings'] = FA::json_decode($def[$key]['settings'], true);
						}
	...
	public function add_placeholder_modifications($def_navi, $slider, $output){
		if(!is_array($def_navi)) $def_navi = FA::json_decode($def_navi, true);
	...
	public function add_placeholder_sub_modifications($css, $handle, $type, $placeholders, $slide, $output){
		...
		if(!is_array($placeholders)) $placeholders = FA::json_decode($placeholders, true);
