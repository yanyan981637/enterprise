...
class RevSliderData {
        ...
        if(empty(Framework::$revslider_animations)){
            ...
            if(!empty(Framework::$revslider_animations)){
                foreach(Framework::$revslider_animations as $ak => $av){
                    Framework::$revslider_animations[$ak]['params'] = FA::json_decode(str_replace("'", '"', $av['params']), true);
                }
			}
	...
	public function get_layer_animations($raw = false){
		...
		$anim = array();
		$anim['in'] = ($raw) ? $in : FA::json_decode($in, true);
		$anim['out'] = ($raw) ? $out : FA::json_decode($out, true);
		$anim['loop'] = ($raw) ? $loop : FA::json_decode($loop, true);
    ...
    protected function _get_base_transitions($raw = false){
        ...
        return ($raw) ? $transitions : FA::json_decode($transitions, true);
    }
}
