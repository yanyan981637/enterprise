...
class RevSliderFolder extends RevSliderSlider {
	...
	public function init_folder_by_id($id){
		...
		if(!empty($folder)){
			...
			$this->settings = (array)FA::json_decode($this->get_val($folder, 'settings', ''));
			$this->params	= (array)FA::json_decode($this->get_val($folder, 'params', ''));
			$this->folder	= true;
			return true;
		}else{
			return false;
	...
	public function add_slider_to_folder($children, $folder_id, $replace_all = true){
		...
		if(!empty($folder)){
			$settings = FA::json_decode($this->get_val($folder, 'settings'), true);
