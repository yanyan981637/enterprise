...
class RevSliderTemplate extends RevSliderFunctions {
	...
	public function copy_slide_to_templates($slide_id, $slide_title, $slide_settings = array()){
		...
		$params = FA::json_decode($duplicate['params'], true);
		$settings = FA::json_decode($duplicate['settings'], true);
	...
	public function get_template_slides(){
		...
		if(!empty($templates)){
			foreach($templates as $key => $template){
				$templates[$key]['params']		= FA::json_decode($template['params'], true);
				//$templates[$key]['layers']	= FA::json_decode($template['layers'], true);
				$templates[$key]['settings']	= FA::json_decode($template['settings'], true);
			}
		}
	...
	public function get_tp_template_slides($sliders = false){
		...
		if(!empty($templates)){
			foreach($templates as $key => $template){
				...
				//$templates[$key]['slider_id'] = FA::json_decode($template['settings'], true);
			}
		}
	...
	public function get_default_template_sliders(){
		...
		if(!empty($sliders)){
			foreach($sliders as $dk => $slider){
				$sliders[$dk]['plugin_require'] = FA::json_decode($sliders[$dk]['plugin_require'], true);
	...
	public function get_tp_template_sliders($uid = false){
		...
		foreach($defaults as $key => $slider) {
			...
			if (isset($slider['required']) && version_compare($availableVersion, $slider['required'], '<')) {
				$skip = true;
			} elseif (isset($slider['plugin_require']) && is_array($pluginRequire = FA::json_decode($slider['plugin_require']))) {
		...
		if(!empty($defaults)){
			...
			foreach($defaults as $dk => $default){
				...
				$defaults[$dk]['plugin_require'] = FA::json_decode($defaults[$dk]['plugin_require'], true);
