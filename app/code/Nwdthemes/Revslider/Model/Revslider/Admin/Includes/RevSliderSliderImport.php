...
class RevSliderSliderImport extends RevSliderSlider {
	...
	public function set_animations(){
		...
		$json_animations = @FA::json_decode($animations, true);
	...
	public function set_dynamic_css_v6(){
		...
		$json_styles = @FA::json_decode($styles, true);
     ...
     */
     public function set_navigations($update_navigation){
		...
		$json_navigations	= @FA::json_decode($navigations, true);
		...
		if(!empty($navigations)){
			foreach($navigations as $navigation){
				...
				if(!isset($navigation['type'])){ //translate navigations to v6 if they are v5
					$_navigations = array();
					$navigation['css'] = FA::json_decode($navigation['css'], true);
					$navigation['markup'] = FA::json_decode($navigation['markup'], true);
					$navigation['settings'] = FA::json_decode($navigation['settings'], true);
	...
	public function process_slider_raw_data(){
		$this->slider_data = @FA::json_decode($this->slider_raw_data, true);
