	...
	public function add_svg_to_zip(){
		...
		foreach ($this->used_svg as $file => $val){
			if (strpos($file, 'http') !== false) {
                $filePath = str_replace('revsliderplugins', 'revslider/plugins', FA::getImagesHelper()->image_url_to_path($file));
                $fileName = basename($file);

                if (is_file($filePath)) {
					if (! $this->usepcl) {
						$this->zip->addFile($filePath, $exportPath . $fileName);
					}else{
						$this->pclzip->add($filePath, PCLZIP_OPT_REMOVE_PATH, str_replace($fileName, '', $filePath), PCLZIP_OPT_ADD_PATH, $exportPath);
					}
				}

                $this->export_data = str_replace(str_replace('/', '\/', $file), str_replace('/', '\/', $exportPath . $fileName), $this->export_data);
			}
		}
	}
			...
			foreach($this->used_images as $file => $val){
				...
				if(strpos($file, 'http') !== false){
					...
					if($remove){ //as its http, remove this from strexport
						...
						*/
						try {
							$unescaped = json_encode(FA::json_decode($this->export_data), JSON_UNESCAPED_SLASHES); // only available from php 5.4
						}
						catch(\Exception $e) {
