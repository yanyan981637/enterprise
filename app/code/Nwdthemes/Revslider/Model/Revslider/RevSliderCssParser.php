...
class RevSliderCssParser extends RevSliderFunctions {
	...
	public function parse_db_to_css($css_array, $nl = "\n\r"){
		...
		foreach($css_array as $id => $attr){
			...
			$attr['advanced'] = FA::json_decode($attr['advanced'], true);
			$styles		= FA::json_decode(str_replace("'", '"', $attr['params']), true);
			...
			//add hover
			$setting = FA::json_decode($attr['settings'], true);
			if(isset($setting['hover']) && $setting['hover'] == 'true'){
				$hover = FA::json_decode(str_replace("'", '"', $attr['hover']), true);
		...
		foreach($css_array as $key => $css){
			if($handle != false){
				if($this->get_val($css_array[$key], 'handle') == '.tp-caption.'.$handle){
					$css_array[$key]['params']	 = FA::json_decode(str_replace("'", '"', $this->get_val($css, 'params')));
					$css_array[$key]['hover']	 = FA::json_decode(str_replace("'", '"', $this->get_val($css, 'hover')));
					$css_array[$key]['advanced'] = FA::json_decode(str_replace("'", '"', $this->get_val($css, 'advanced')));
					$css_array[$key]['settings'] = FA::json_decode(str_replace("'", '"', $this->get_val($css, 'settings')));
	...
	public function get_captions_sorted(){
		...
		foreach($styles as $style){
			$setting = FA::json_decode($this->get_val($style, 'settings'), true);
	...
	public function get_database_classes($adv = false){
		...
		if(!empty($css)){
			foreach($css as $k => $v){
				if($adv === true){
					$css[$v['handle']]['hover']		= FA::json_decode($this->get_val($v, 'hover', ''), true);
					$css[$v['handle']]['params']	= FA::json_decode($this->get_val($v, 'params', ''), true);
					$css[$v['handle']]['settings']	= FA::json_decode($this->get_val($v, 'settings', ''), true);
				}else{
					unset($css[$v['handle']]['hover']);
					unset($css[$v['handle']]['params']);
					unset($css[$v['handle']]['settings']);
				}
				$css[$v['handle']]['advanced'] = FA::json_decode($this->get_val($v, 'advanced', ''), true);
			}
		}
