...
class RevSliderCache extends RevSliderFunctions {
	...
	public function do_additions($additions, $output){
			...
			foreach($t_actions as $_action => $t_a){
				if(!empty($t_a)){
					...
					foreach($t_a as $t_sa){
						...
						FA::add_action($_action, array($this, 'print_addition'));
					}
				}
			}
		}
			...
			foreach($t_filters as $_filter => $t_a){
				if(!empty($t_a)){
					foreach($t_a as $t_sa){
						...
						FA::add_filter($_filter, array($this, 'print_addition'));
					}
				}
			}
		}
		...
		if(!empty($t_special)){
			...
			if(!empty($_rs_css_collection)){
				Framework::$rs_css_collection = $_rs_css_collection;
			}
		...
		FA::do_action('revslider_do_cache_additions', $additions, $output);
	}
	...
	public function set_full_transient($transient, $sid, $content){
		...
		$add = json_encode($add);
		FA::set_transient($transient, $add, 60*60*24*7);
	...
	public function print_addition(){
		$html = $this->get_val($this->output_html, FA::current_filter());
		if(is_array($html)){
			if(!empty($html)){
				...
				echo (FA::current_filter() === 'wp_print_footer_scripts') ? Framework::RS_T.'</script>'."\n" : '';
			}
		}else{
			echo $html;
		}
