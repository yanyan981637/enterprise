...
class Post implements ItemProviderInterface
{
    ...
    {
        $items = [];
        if ($this->dataHelper->getConfig('sitemap/post/enabled')) {
            ...
            $items = array_map(function ($item) use ($storeId) {
                return $this->itemFactory->create([
                    ...
                    'priority' => $this->dataHelper->getConfig('sitemap/post/priority'),
                    'changeFrequency' => $this->dataHelper->getConfig('sitemap/post/changefreq'),
                ]);
            }, $collection->getItems());
        }
