...
class UpdateOrderInfo implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        try {
            ...
            $orderInfoData = $weSupplyOrder->getInfo() ?? '';

            $existingOrderXml = simplexml_load_string($orderInfoData, 'SimpleXMLElement');
            ...
            $this->setOrderUpdatedAt($observer, $weSupplyOrder, $orderData);
     ...
     * @param $orderData
     */
    private function setOrderUpdatedAt($observer, $weSupplyOrder, &$orderData)
    {
        if (!empty($observer->getData('origUpdatedAt')) && true === $observer->getData('origUpdatedAt')) {
            $weSupplyOrder->setUpdatedAt($orderData['LastModifiedDate']);
            $orderData['OrderModified'] = $orderData['LastModifiedDate'];
        }
    }
}
